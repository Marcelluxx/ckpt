...
async def test_save_snapshot_success(mocker: MockerFixture) -> None:
    ...
    mock_save = mocker.patch("ckpt.mcp_server.save_checkpoint", return_value="/mock/path/a1b2c3d4.json")
...
async def test_save_snapshot_llm_config_error(mocker: MockerFixture) -> None:
    ...
    mocker.patch("ckpt.mcp_server.create_snapshot", return_value=mock_checkpoint)
    mocker.patch("ckpt.mcp_server.generate_mental_map", side_effect=ConfigError("LLM not configured"))
    mock_save = mocker.patch("ckpt.mcp_server.save_checkpoint", return_value="/mock/path/a1b2c3d4.json")
...
async def test_save_snapshot_llm_error(mocker: MockerFixture) -> None:
    ...
    mocker.patch("ckpt.mcp_server.create_snapshot", return_value=mock_checkpoint)
    mocker.patch("ckpt.mcp_server.generate_mental_map", side_effect=LLMError("Gemini call failed"))
    mock_save = mocker.patch("ckpt.mcp_server.save_checkpoint", return_value="/mock/path/a1b2c3d4.json")
...
async def test_save_snapshot_direct_mental_map(mocker: MockerFixture) -> None:
    ...
    result = await save_snapshot("Test checkpoint message", mental_map="Directly provided mental map")
